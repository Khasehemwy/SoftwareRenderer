...
void Draw_Scene(Renderer& renderer,Renderer& renderer_ground)
{
	...
	renderer_ground.transform.view = matrix_lookat(renderer.camera->pos, renderer.camera->target, renderer.camera->up);
...
int main()
{
	...
	while (window.screen_exit[0] == 0 && window.screen_keys[VK_ESCAPE] == 0) {
		...
		//画盒子 & 地面
		Draw_Scene(renderer, renderer_ground);
