...
{
	return F0 + (vector_t(max(roughness, F0.x), max(roughness, F0.y), max(roughness, F0.z)) - F0) * pow(1.0 - cosTheta, 5.0);
}
...
color_t Renderer_PBR::PS(vertex_t* v)
{
	...
	for (int i = 0; i < lights.size(); i++) {
		...
		vector_t F = fresnelSchlickRoughness(CMID(vector_dot(N, V), 0.0, 1.0), F0, roughness);
	...
	vector_t kS = fresnelSchlickRoughness(CMID(vector_dot(N, V), 0.0, 1.0), F0, roughness);
	vector_t kD = 1.0 - kS;
	kD = kD * (1.0 - metallic);
	vector_t irradiance = cube_textures["irradiance"]->Read(N);
	vector_t diffuse = irradiance * albedo;
	vector_t ambient = vector_dot(kD, diffuse) * ao;
